...
pub struct Home {
    state: State,
    link: ComponentLink<Self>,         // Allows us to register callback to trigger the update lifecycle method
...
impl Component for Home {
    ...
    // Update is the place where the logic to update the component, in this case `State`, or perform
    // side-effects(like network requests) exist. It is invoked using a `Message` enum that contains
    // all the action the component supports. When we return `true` from this method, the component
    // is re-rendered.
    ...
    fn view(&self) -> Html {
        ...
        if !self.state.get_products_loaded {
            ...
            html! {
                <div>
                  <div class="navbar">
                    <div class="navbar title">{"Starter"}</div>
